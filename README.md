...
* 연속형 변수를 `KMeans`로 군집화해 새로운 범주형 변수로 추가하였다. 해당 변수의 유의성이 낮더라도, Tree 기반 모형에서는 성능에 큰 영향을 주지 않음을 고려하였다.
...
* 변수 중요도:
  ...
  * 메타데이터 측면에서 예산과 스릴러 장르의 중요도가 높게 나타났다.
...
* 결론:
  ...
  * 많은 예산이 투입된 할리우드식 액션의 시각 효과는 보편적으로 선호되는 영역이므로, 국내 시장을 겨냥하려면 청각적 요소에 대한 추가 투자가 필요하다.
 
  * 대형 IP는 국내 특화형과는 거리가 멀다. 반대로 글로벌 시장에 대응하려면 검증된 IP가 필수적이다. 글로벌 시장을 목표로 한다면 초기 기획 단계에서 장기적 IP 투자와 트랜스미디어 전략 수립이 권장된다.
 
  * 리뷰의 양극화는 논쟁과 호기심을 유발해 의외의 성공으로 이어지는 경향이 있다 ([Louposet al.](https://link.springer.com/article/10.1007/s11002-023-09665-8)). 부정 감성 키워드가 국내 특화율을 높인 것은 이 '화제성'과 맞물린 결과로 해석된다. 국내 시장을 노린다면 버즈 마케팅으로 바이럴 효과를 극대화할 필요가 있다.

### 개선점

* 제안서의 타당성이 부족하였다. 국내·해외 작품을 동시에 분석하였다면 두 시장을 모두 파악할 수 있었을 것이다. "한국 문화가 주도적이므로 해외 작품이 한국에 맞춰야 한다"는 설명은 근거가 빈약하였으며, 결론 또한 사업적 확장 가치가 부족하였다.

* 제안서에 `LGBMClassifier` 사용이 명시되어 있어, Transformer 기반의 언어 모형을 예측에 직접 활용할 수 없었다.

* 일반 구성원이라는 위치와 확정된 제안서로 인해 프로젝트 방향을 수정하기 어려웠다. 리더의 결정과 팀의 일관성을 존중하는 것이 중요하다고 판단하였고, 그 결과 정해진 범위 내에서 최선의 성과를 도출할 수 있었다.

* 예상과는 다르게 통계 전문 심사위원이 없었으며, 발표가 지나치게 기술적이었다. 비전문적인 핵심 메시지조차 충분히 전달되지 못했고, 이는 본선 탈락의 주 원인으로 판단된다.
...
* 평가 데이터: 2024년 6월 16일부터 2025년 5월 24일까지, 총 193종의 `영업장명_메뉴명`의 `매출수량`이 부분적으로 제공되었다. 28일치 관측값과 7일치 공란으로 구성된 35일 단위의 평가 샘플 파일이 총 10개 제공되었다.
...
* `매출수량`의 Lag와 평균·표준편차·사분위수를 추가로 도입한 후 `LGBMRegressor`, `XGBRegressor`, `RandomForestRegressor`를 학습하였다. 0-Inflated Count Data에 대응하기 위해, `LGBMRegressor`, `XGBRegressor`는 Tweedie Regression을 목적함수로 설정하였다. `RandomForestRegressor`
