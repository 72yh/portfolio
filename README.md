...
* 학습과 평가 데이터 모두 부족해 일반화 성능을 신뢰할 수 없다. 분자 데이터를 추가로 확보해 GNN의 Head를 Fine-Tuning했다면 좋은 성능을 달성할 수 있었을 것으로 보인다.
...
* 리더로서 데이터 유출 기준을 명확하게 세우지 못하였다. 예를 들어, 예측 시 당일 기상 자료는 사용할 수 있었지만, 당일 인구 정보의 사용 가능 여부는 불분명하였다. 이러한 이유로 일부 파생 변수의 시점 기준이 일관되지 않았으며, 당시 리더십을 발휘해 이를 명확히 통일했다면 더 나은 결과로 이어질 수 있었을 것이다.
...
* 상위권 경쟁자들과 동일한 기반 모형, 학습률(Learning Rate), 스케줄러(Scheduler)를 적용하였으나, 학습 속도가 느려 충분한 Epoch을 확보하지 못해 수렴이 완료되지 않은 모형을 제출하게 되었다.
...
* 일부 Windows 환경에서는 `DataLoader`의 `num_workers`를 0보다 크게 설정할 경우 충돌이 발생하는 문제가 알려져 있다. 해당 프로젝트에서도 동일한 문제가 발생하여 Visual Studio Code의 셀 마커(`# %%`) 기능으로 우회하였다.
