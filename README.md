...
### 주요 역할
* 기상 요 외에도 신고 건수에 영향을 미치는 요인들을 사전 조사한 뒤, 관련 자료를 수집하였다.
* 데이터 유출을 방지하도록 설계된 목표 기반 통계량(TargetEncoder) 파이프라인을 구축하고, 시계열 특성에 적합한 교차 검증 전략을 수립하였다.
* 개별 모델의 하이퍼파라미터 최적화 및 최종 앙상블 구성을 담당하였다.
