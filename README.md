# [장동은] 데이터 사이언티스트 경력 기술서

* 기술 역량은 저장소 최상위의 `SUMMARY.pdf`에서 확인하실 수 있습니다.

* 프로젝트는 역순(최신순)으로 나열되어 있습니다.

* 저장소 내 각 폴더는 기재된 프로젝트 제목과 동일한 이름으로 구성되어 있으며, 프로젝트 수행에 활용된 파이썬 코드(`.py`, `.ipynb`), 일부 경량 모형의 가중치, 정형 데이터 등이 포함되어 있습니다.
...
* Paper Recommender는 이러한 탐색 부담을 줄이기 위해 개발한 웹 서비스로, 사용자가 arXiv 논문 URL을 입력하면 내용이 유사한 논문 10편을 추천한다. 사용자는 필요에 따라 카테고리와 날짜 범위를 지정해 추천 범위를 조정할 수 있다.
...
* 프로젝트의 문제 정의와 핵심 기술 방향을 직접 설계하였다.
   ...
   * 논문의 메타데이터를 언어 모델로 벡터화한 뒤, 벡터 유사도를 기준으로 관련 논문을 추천하는 구조를 기획하였다.
