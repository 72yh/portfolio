...
* 제공된 데이터에 일부 품질 문제가 존재하였다:
  ...
  * 평가 데이터 역시 신고가 있던 날만 포함되어 있었으며, 대회의 목적이 다소 불분명하였다.
...
* 국내 작품만으로는 파악하기 어려운 한국 시장 고유의 감성을 파악하고자 하였다.

* 글로벌 성과 대비 국내 선호가 두드러지는 해외 영화의 리뷰를 분석하여 해당 감성을 도출하였다.

### 주요 역할

* 프로젝트 후반에 합류하여 팀의 스토리라인을 헤치지 않는 선에서 분석 오류를 수정하고 코드 구조를 재정비하였다.

* 본선 발표 자료를 준비하고 발표하였다.
...
* 평가 데이터의 부재로 5-Fold 교차검증을 통해 모형의 성능을 검증하였다.

### 분석 방안

* 국내 매출 비중이 일정 수준 이상, 평균 대비 글로벌 매출이 일정 수준 이하인 해외 영화를 '국내 특화형'으로 정의하고, 이진 분류를 수행하였다.  

* 팀 제안서에 따라, 해석에 용이한 하드코딩 감성 사전, TF-IDF, `LGBMClassifier`를 활용하였다. 감성 사전은 ChatGPT와 Gemini에 리뷰 데이터를 입력하여 생성하였다.
...
* Tree 기반 모형의 문맥 파악력을 보완하기 위해, 사전학습 BERT를 활용하여 리뷰의 긍정·부정 확률을 산출하고 입력 변수로 사용하였다.

* 변수 중요도와 SHAP(SHapley Additive exPlanations) 기반 해석·인사이트 도출을 우선 목표로 설정하였다.
