...
* 제공된 데이터에 일부 품질 문제가 존재하였다:
  ...
  * `OUTER JOIN`이 아닌 `INNER JOIN` 방식으로 결합된 소방·기상 데이터가 제공되어, 신고가 일어나지 않은 날짜의 정보는 데이터에서 제외되었다. 이로 인해 신고 건수가 0인 날은 예측할 수 없으며, 평가 데이터 역시 신고가 있는 날만 포함된 것으로 보인다. 결과적으로 '신고가 발생한 날의 건수'만을 예측하는 구조가 되어, 대회의 취지가 다소 불분명하다.
