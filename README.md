...
* 모호한 데이터 유출 기준으로 인해 팀 내 혼선이 발생하였다. 예를 들어, 예측 시 당일 기상 자료는 사용할 수 있었지만, 당일 인구 정보의 사용 가능 여부는 불분명했다. 이러한 이유로 일부 파생 변수의 시점 기준이 일관되지 않았으며, 당시 리더십을 발휘해 이를 명확히 통일했다면 더 나은 결과로 이어질 수 있었을 것이다.

* 신고가 없는 일자의 기상 자료까지 확보했다면, 조건부 예측이 아닌 전반적인 신고 건수 예측이 가능했을 것이다.
...
* 데이터 규모의 한계로 딥러닝 모델은 제외하였으나, 시도했다면 경쟁력 있는 성능을 보였을 가능성도 있다.
