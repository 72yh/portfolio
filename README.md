...
* 앙상블 모형의 악성 클래스 판별 임계값 산출 시, 각 모형의 출력값을 메타 특성으로 활용하여 교차 검증을 수행하였다. 검증 폴드를 포함한 전체 예측값을 입력으로 사용하는 방식은 관행적으로 널리 사용되지만, 데이터 유출에 해당하며 F1 Score가 과대평가될 수 있다는 한계가 있다.
